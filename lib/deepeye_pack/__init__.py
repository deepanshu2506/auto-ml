...
class deepeye(object):
    ...
    def table_info(self, name, column_info, *column_info2):
        ...
        if isinstance(column_info, list) and isinstance(column_info2[0], list):
            ...
            raise TypeError(
                "unsupported argument types (%s, %s)"
                % (type(column_info), type(column_info2))
            )
    ...
    def from_csv(self, path):
        ...
        self.csv_dataframe = pd.DataFrame(
            pd.read_csv(self.csv_path, header=0, keep_default_na=False)
    ...
    def csv_handle(self, instance):
        ...
        for i in range(instance.column_num):
            ...
            )  # getType is a static method
        instance.tables[0].origins = [i for i in range(instance.tables[0].column_num)]

        instance.tuple_num = instance.tables[0].tuple_num = table_origin.shape[
            0
        ]  # the number of rows
        for i in range(instance.tables[0].column_num):
            if (
                instance.tables[0].types[i] == 3
            ):  # if there is date type column in csv,convert into datetime format
        ...
        for i in range(len(table_origin.columns)):
            table_origin.rename(
                columns={table_origin.columns[i]: in_column_name[i]}, inplace=True
            )
        instance.tables[
            0
        ].D = (
            table_origin.values.tolist()
        )  # dataframe to list type and feed to D(where to store all the table info )
        return instance
    ...
    def csv_handle_changedate(self, col_name, col_type):
        ...
        if col_type == "date":
            ...
            table[col_name] = pd.to_datetime(
                table[col_name].apply(lambda x: str(x) + "/1/1")
            ).dt.date
    ...
    def rank_generate_all_views(self, instance):
        ...
        instance.addTables(
            instance.tables[0].dealWithTable()
        )  # the first deal with is to transform the table into several small ones
        # print(instance.table_num, instance.view_num)
        begin_id = 1
    ...
    def html_output(self, order, view, mode):
        ...
        if mode == "single":
            ...
            grid.render(
                "./html/" + self.table_name + "/" + filename
            )  # the grid is added in a new file
