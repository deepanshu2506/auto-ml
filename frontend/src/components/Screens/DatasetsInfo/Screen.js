...
const DatasetInfoScreen = (props) => {
  ...
  const [colDescModalOpen, setColDescModalOpen] = useState(false);
  ...
  const openColumnDescription = (dataset, column) => {
    console.log(column);
    setcolumnDetail(column);
    setColDescModalOpen(true);
  };
  ...
  return (
    <Container className={`${styles.screen}  pt-3 pl-4 `} fluid>
      <ImputeModal
        key={columnDetail.column_name}
        passIsImputed={setIsImputed}
        modalOpen={modalOpen}
        datasetId={datasetId}
        columnDetail={columnDetail}
      ></ImputeModal>
      <ColumnDescriptionModal
        modalOpen={colDescModalOpen}
        datasetId={datasetId}
        column={columnDetail}
        onClose={() => setColDescModalOpen(false)}
      />
                ...
                <Row>
                  <Col sm={6} lg={3}>
                    <div className={`${styles.cardcounter} ${styles.primary}`}>
                      ...
                      <br />
                      <span
                        className={`${styles.countname} ${styles.countname2}`}
                      >
              ...
              <Row className={styles.functions}>
                ...
                <Col md={3}>
                  <Link to={`${location.pathname}/visualization`}>
                    <Button className="visualizebtn" block>
                      Visualization
                    </Button>
                  </Link>
                </Col>
                <Col md={3}>
                  <Link to={`/dataset/model_selection/${info.id}`}>
                    <Button className="modelbtn" block>
                      Create Model
                    </Button>
                  </Link>
                </Col>
                <Col md={3}>
                  <Link to={`${location.pathname}/preview`}>
                    <Button className="previewbtn" block>
                      Dataset Preview
                    </Button>
                  </Link>
                </Col>
                        ...
                        <tr key={i}>
                          ...
                          >
                            ...
                            <Button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                openColumnDescription(datasetId, column);
                              }}
                            >
                              <FaInfoCircle />
                            </Button>
                          ...
                          <td style={{ width: "24%", textAlign: "center" }}>
                            <Button
                              style={{ padding: "0.1em 0.5rem" }}
                              variant="primary"
                              onClick={() => openModal(datasetId, column)}
                            >
                              <i
                                ...
                                {column.column_Type === "continous" ? (
                                  ...
                                  <td colSpan="3">
                                    {column.metrics.unique_values < 15 ? (
                                      ...
                                      <h6>
                                        More than 15 unique values to show in
                                        graph
                                      </h6>
                                    )}
                                  </td>
...
const ColumnDescriptionModal = ({ column, modalOpen, ...props }) => {
  const [state, setState] = useState({});

  const [isEditting, setEditting] = useState(!column.column_description);
      ...
      data: [
        {
          ...
          description: state.col_description,
        },
      ],
    };
    try {
      await API.json.post(
        apiURLs.dataset.colDescription(props.datasetId),
        data
      );
  ...
  const onEdit = () => {
    ...
    if (column.column_description) {
      setState((prev) => ({
        ...prev,
        col_description: column.column_description,
      }));
    }
  };

  useEffect(() => {
    setEditting(!column.column_description);
  }, [column]);
    ...
    >
      <Row>
        {column.column_description ? (
          <Col>{column.column_description}</Col>
        ) : (
