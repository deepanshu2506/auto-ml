...
const ListDatasetScreen = (props) => {
  ...
  return (
    <Container>
      ...
      <Row>
        <Col><span>My Datasets</span></Col>        
        <Col>
          <Link to={{pathname: `/dataset/create`}}>
            <Button style={{padding:"0.1rem 0.5rem"}} className="float-right" variant="outline-success"> Create a dataset <i class="fa fa-plus" aria-hidden="true"></i></Button>
          </Link>
        </Col>
      </Row>
      ...
      <Row style={{alignItems:'center',flexDirection:'column',}}>
        ...
        info && info.data.map(column=>
     [ <tr> 
        <td style={{width:'10%'}}>{index=index+1}</td>
        <td style={{width:'25%'}}>{column.dataset_name}</td>
        <td style={{width:'25%'}}>{column.created_at.slice(0,-14).trim()}</td>
        <td style={{width:'10%'}}>{column.type}</td>
        <td style={{width:'15%',textAlign:'center'}}>
        <Link to={{pathname: `/datasets/${column.id}`}}>
        <Button style={{padding:"0.1em 0.5rem"}} variant="primary"><i class="fa fa-info-circle" aria-hidden="true"></i></Button>
        </Link></td>
        <td style={{width:'15%',textAlign:'center'}}>
        <Link to={{pathname: `/datasets/${column.id}`}}>
        <Button style={{padding:"0.1em 0.5rem"}} variant="danger"><i class="fa fa-trash" aria-hidden="true"></i></Button>
        </Link></td>
        </tr>
      ]
      )) 
